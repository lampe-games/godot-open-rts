...
static func find_valid_position_radially(
	starting_position: Vector3, radius: float, navigation_map_rid, scene_tree
):
...
static func find_valid_position_radially_yet_skip_starting_radius(
	...
	if (
		...
		and (not navigation_map_rid or _is_agent_placement_position_valid(
	...
	for ring_number in range(starting_offset, 999999):
		...
		for radial_position in radial_positions:
			if not navigation_map_rid or _is_agent_placement_position_valid(
